...
class BNN_KDE(nn.Module):
    def __init__(self, emp_samples, input_size=1, num_nodes=2, output_size=1, alpha=1., beta=5., kl_beta=1., initial_rho=-3.):
        super(BNN_KDE, self).__init__()
        self.register_buffer('emp_samples', emp_samples)
        self.log_kde_rho = nn.Parameter(initial_rho * torch.ones(emp_samples.shape[0], 1))
    ...
    def kde(self):
        ...
        covariances = torch.eye(problem_dim)[None].repeat(self.log_kde_rho.shape[0], 1, 1)
        component_covariances = kde_var[:, None].repeat(1, problem_dim, problem_dim) * covariances
        gaussian_components = D.MultivariateNormal(loc=self.emp_samples,
                                                   covariance_matrix=component_covariances)
